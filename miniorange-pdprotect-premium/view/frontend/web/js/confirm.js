/* MiniOrange PDProtectPremium - Confirm Action page JS (frontend)
 *
 * Overrides PDProtect::js/confirm.js to fix a timing bug:
 * The free module's IIFE captured window.mopdpConfirmConfig at script-load
 * time (in <head>), before the phtml template in <body> had a chance to
 * set it — so ACTION_URL was always '' and every XHR posted to the current
 * page URL instead of the real action endpoint.
 *
 * Fix: read window.mopdpConfirmConfig inside mopdpConfirmAction() at
 * *call time*, not at load time.
 */

(function () {

    window.mopdpConfirmAction = function () {
        /* Read config at call time so the values set by the phtml <script>
         * block in the page body are already present. */
